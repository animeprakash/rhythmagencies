...
<?php
		...
		$query_b="select amount,sno from ticketbooking where sno=(select max(sno) from ticketbooking)";
		$result3 = mysqli_query($con, $query_b);
		while($rowdata = mysqli_fetch_assoc($result3)) {?>
			document.getElementById("totalamount").innerHTML=arr.length*<?php echo $rowdata['amount']; ?>;
			document.getElementById("totalamount2").innerHTML=arr.length*<?php echo $rowdata['amount']; ?>;
<?php } ?>
	});
	});	
	...
	{	
		window.location.href = 'fullsetwhatsapp.php?fullsetinfo=%20'+(arr.toString());
	}
...
<?php
		include("database.php");
		$querytwo="select fullticketno from fullticket";
...
<section>
		...
		<div class="container ">
			<div class="select justify-content-between">
				<div class="float-left butt selectee" ><div class="flex btn bg-warning" >Selected Tickets<div id="numcount" class="btn bg-warning ">0</div></div></div>
				<div class="text-center selectee"><input type="submit" name="fullsetconfirm" id="fullsetconfirm" class="btn bg-success text-white"  data-toggle="modal" data-target="#myModal" value="CONFIRM TICKET" onclick="selectednum()"></div>
				<div class="float-right butt selectee2"><div class="flex btn bg-warning text-right" >Total Amount &#8377 <div id="totalamount" class="btn w-25 bg-warning nums"> 0</div></div>	</div>		
			</div>
