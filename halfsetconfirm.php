...
<?php
		...
		$query_b="select round(amount/2),sno from ticketbooking where sno=(select max(sno) from ticketbooking)";
		$result3 = mysqli_query($con, $query_b);
		while($rowdata = mysqli_fetch_assoc($result3)) {?>
			document.getElementById("totalamount").innerHTML=arr.length*<?php echo $rowdata['round(amount/2)']; ?>;
			document.getElementById("totalamount2").innerHTML=arr.length*<?php echo $rowdata['round(amount/2)']; ?>;
<?php } ?>
	});
	});	
	...
	{	
		window.location.href = 'halfsetwhatsapp.php?halfsetinfo=%20'+(arr.toString());
	}
...
<section class="form-inline  d-flex" class="wid">
...
<div style="background-color:#FFE4B5;" class="firstwid">	
	...
	<?php
		...
		echo "<html> <h4 style='margin-left:5%;'><script >";
		echo "document.write(firsthalf)";
		echo "</script></h4></html>";
	?>
<?php	ini_set('display_errors', 0);
		include("database.php");
...
?>		
	...
	?>
		<div class="col-14 text-center  btn" id="column" >
